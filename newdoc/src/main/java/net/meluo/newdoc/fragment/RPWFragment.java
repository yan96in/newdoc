...
public final class RPWFragment extends BaseFragment {
	...
	public static RPWFragment newInstance() {

		RPWFragment fragment = new RPWFragment();
		return fragment;
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		layout = (RelativeLayout) inflater.inflate(R.layout.f_rpw, null);
	...
	private void getCheck() {
		...
		Ion.with(this.getActivity(),
				...
				.setMultipartParameter("mobile",
						NDApp.getInstance().user.getMobile())
	...
	private void rPW() {
		...
		if (Checking.isNullorBlank(etPw.getText().toString())) {
			T.showShort(RPWFragment.this.getActivity(), "请输入密码");
			return;
		}

		if (Checking.isNullorBlank(etRpw.getText().toString())) {
			T.showShort(RPWFragment.this.getActivity(), "请重复密码");
			return;
		}
		if (!etRpw.getText().toString().equals(etPw.getText().toString())) {
			T.showShort(RPWFragment.this.getActivity(), "请确认两次输入密码一致");
			return;
		}
		if (Checking.isNullorBlank(etCheck.getText().toString())) {
			T.showShort(RPWFragment.this.getActivity(), "请输入验证码");
			return;
		}
		try {
			Ion.with(this.getActivity(),
					...
					.setCallback(new FutureCallback<String>() {
						...
						public void onCompleted(Exception arg0, String data) {
							...
							if (Checking.isNullorBlank(data)) {
								T.showShort(RPWFragment.this.getActivity(),
										"网络错误，请稍后重试");
								return;
							}
							try {
