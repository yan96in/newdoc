...
public final class FindBackPWFragment extends BaseFragment {

	Context mContext;
	TitleBar mTitleBar;

	EditText etId, etPhone, etPw, etRpw, etCheck;

	Button btnCheck, btnSignUp;

	private TimeCount time;

	public static FindBackPWFragment newInstance() {

		FindBackPWFragment fragment = new FindBackPWFragment();
		return fragment;
	}
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		layout = (RelativeLayout) inflater.inflate(R.layout.f_fbpw, null);
	...
	private void getCheck() {
		if (Checking.isNullorBlank(etId.getText().toString())) {
			T.showShort(FindBackPWFragment.this.getActivity(), "请输入手机号码");
			return;
		}
		...
		Ion.with(this.getActivity(),
				...
				.setMultipartParameter("mobile", etId.getText().toString())
				.setMultipartParameter("f", "forget").asString()
	...
	private void rPW() {
		...
		if (Checking.isNullorBlank(etPw.getText().toString())) {
			T.showShort(FindBackPWFragment.this.getActivity(), "请输入密码");
			return;
		}

		if (Checking.isNullorBlank(etRpw.getText().toString())) {
			T.showShort(FindBackPWFragment.this.getActivity(), "请重复密码");
			return;
		}
		if (!etRpw.getText().toString().equals(etPw.getText().toString())) {
			T.showShort(FindBackPWFragment.this.getActivity(), "请确认两次输入密码一致");
			return;
		}
		if (Checking.isNullorBlank(etCheck.getText().toString())) {
			T.showShort(FindBackPWFragment.this.getActivity(), "请输入验证码");
			return;
		}
		try {
			Ion.with(this.getActivity(),
					PreferenceConstants.BASE_URL0 + "/Common/1/forget")

					.setMultipartParameter("newMd5",
					...
					.setCallback(new FutureCallback<String>() {
						...
						public void onCompleted(Exception arg0, String data) {
							...
							if (Checking.isNullorBlank(data)) {
								T.showShort(
										FindBackPWFragment.this.getActivity(),
										"网络错误，请稍后重试");
								return;
							}
							try {
								...
								if (o.getInt("retcode") == 0) {
									// 修改密码成功
									T.showShort(mContext, "找回成功，请返回登录页面登录。");
									FindBackPWFragment.this.getActivity()
